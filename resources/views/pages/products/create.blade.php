    ...
    <div class="container mx-auto mt-10 max-w-2xl">
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" rows="4" required>{{ old('description') }}</textarea>
                @error('description')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="material" name="material" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" value="{{ old('material') }}" required>
                @error('material')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="production_time_days" name="production_time_days" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" value="{{ old('production_time_days') }}" min="1" required>
            ...
            <div class="mb-4">
                ...
                <select id="complexity" name="complexity" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                    ...
                    @foreach ($complexityLevels as $level)
                        <option value="{{ $level->value }}" {{ old('complexity') == $level->value ? 'selected' : '' }}>
                            {{ ucfirst($level->value) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="durability" name="durability" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                    ...
                    @foreach ($durabilityLevels as $level)
                        <option value="{{ $level->value }}" {{ old('durability') == $level->value ? 'selected' : '' }}>
                            {{ ucfirst($level->value) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea id="unique_feature" name="unique_feature" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" rows="3" required>{{ old('unique_feature') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="price_credit" name="price_credit" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" value="{{ old('price_credit') }}" min="0" step="0.01" required>
                @error('price_credit')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="has_external_link" class="flex items-center">
                    <input type="checkbox" id="has_external_link" name="has_external_link" class="mr-2" {{ old('has_external_link') ? 'checked' : '' }}>
